...
export function useTimerSettings() {
  const [delaySeconds, setDelaySeconds] = useState<number>(2454);
  ...
  const fetchTimerSettings = async () => {
    if (!supabase) {
      console.warn('Supabase not configured, using default 2454 seconds');
      setDelaySeconds(2454);
      return;
    }
    ...
    try {
      ...
      if (data) {
        ...
        console.log('No timer settings found, using default 2454 seconds');
        setDelaySeconds(2454);
      }
    } catch (err) {
